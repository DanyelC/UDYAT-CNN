...
def main_fun(args, ctx):
    ...
    for train, test in kfold.split(data, labels):
        ...
        layers.Conv2D(256,3, activation='tanh',padding='same'), #1024

        layers.MaxPooling2D(pool_size=(2, 2), strides=(1,1)), #OVERLAPPING POOLING
        ...
        layers.Conv2D(512,2, activation='tanh',padding='same'), # 4
        ...
        model.compile(optimizer=keras.optimizers.Adam(learning_rate=0.0002),loss = 'binary_crossentropy',metrics=METRICS)
        model.build()
        model.summary()
    
        callbacks = [tf.keras.callbacks.EarlyStopping(monitor='loss', mode='min', patience=100, restore_best_weights=True)] #EarlyStopping
        ...
        with open("caminhopara_results.txt",'a') as file:
